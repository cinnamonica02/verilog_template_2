...
`A`, followed by four row-major values for matrix `B`. Eight time-multiplexed
multiply cycles later, the four row-major values of `C = A x B` are presented
on `uo_out`, one per clock. Results are saturated to signed INT8. Reset starts
a new matrix operation, and the bidirectional pins are unused.
